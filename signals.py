"""
signals.py — Signalgenerering med candlestick-mönster + RSI/MACD-bekräftelse.
"""
...
from patterns import (
    detect_morning_star, detect_hammer, detect_bullish_engulfing,
    detect_dark_cloud_cover, detect_tweezer_top, detect_evening_star,
)
...
def _macd(closes, fast, slow, sig):
    ...
    return {
        ...
        "histogram": float(histogram.iloc[-1]), "golden": golden, "death": death,
        "bullish": float(macd_line.iloc[-1]) > float(signal_line.iloc[-1]),
    }


def _bollinger(closes, period, std):
    ...
    return {
        "upper": float(upper.iloc[-1]), "middle": float(sma.iloc[-1]),
        "lower": float(lower.iloc[-1]), "price": price,
...
def _confirmed_buy(rsi, macd, cfg):
    confirms = []
    ...
    if macd["bullish"]:
        ...
        confirms.append(f"RSI<50 ({rsi:.1f})")
    ok = rsi < cfg.RSI_OVERBOUGHT and (macd["bullish"] or rsi < 50)
    return ok, confirms


def _confirmed_sell(rsi, macd, cfg):
    confirms = []
...
def get_signal(symbol, bars, cfg):
    ...
    # Steg 1 — Candlestick-mönster med bekräftelse
    for pattern_name, detector in [("Morning Star",      detect_morning_star),
        ...
        if detector(bars):
            ...
            if ok:
                return {"symbol": symbol, "direction": "BUY", "score": 2,
                        "buy_signals": [pattern_name] + confirms, "sell_signals": [],
                        "pattern": pattern_name, "rsi": rsi, "macd": macd, "bb": bb}
    ...
    for pattern_name, detector in [("Evening Star",     detect_evening_star),
        ...
        if detector(bars):
            ...
            if ok:
                return {"symbol": symbol, "direction": "SELL", "score": -2,
                        "buy_signals": [], "sell_signals": [pattern_name] + confirms,
                        "pattern": pattern_name, "rsi": rsi, "macd": macd, "bb": bb}

    # Steg 2 — Fallback: ren RSI/MACD/BB
    buy_signals  = []
    sell_signals = []
    ...
    return {"symbol": symbol, "direction": direction,
            ...
            "pattern": None, "rsi": rsi, "macd": macd, "bb": bb}
